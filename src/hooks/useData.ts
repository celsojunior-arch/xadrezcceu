...
export const useData = () => {
  ...
  const editarResultadoDesafio = async (confrontoId: string, novoResultado: '1-0' | '0-1' | '0-0'): Promise<void> => {
    ...
    try {
      ...
      // Se já tinha resultado, reverter as mudanças anteriores primeiro
      if (confronto.resultado) {
        ...
        // Reverter ratings aos valores anteriores
        ...
        const { error: revertBrancasError } = await supabase
          .from('players')
          .update({ 
        ...
        if (revertBrancasError) throw revertBrancasError;
        
        const { error: revertPretasError } = await supabase
          .from('players')
          .update({ 
        ...
        if (revertPretasError) throw revertPretasError;
      ...
      // Calcular novas mudanças de rating baseadas nos ratings snapshot
      ...
      // Aplicar novas mudanças baseadas nos ratings snapshot originais
      const finalRatingBrancas = Math.round(confronto.ratingBrancasSnapshot + newChangeBrancas);
      const finalRatingPretas = Math.round(confronto.ratingPretasSnapshot + newChangePretas);
      
      // Atualizar ratings dos jogadores
      const { error: updateBrancasError } = await supabase
        .from('players')
        .update({ 
      ...
      if (updateBrancasError) throw updateBrancasError;
      
      const { error: updatePretasError } = await supabase
        .from('players')
        .update({ 
      ...
      if (updatePretasError) throw updatePretasError;

      // Criar novo histórico de rating
      const { error: historyError } = await supabase
        .from('rating_history')
        .insert([
          {
      ...
      // Atualizar o confronto com o novo resultado
      const { error } = await supabase
        .from('desafio_confrontos')
        .update({
      ...
      alert('Erro ao editar resultado: ' + (error instanceof Error ? error.message : 'Erro desconhecido'));
    } finally {
      setLoading(false);
    }
  };
